...
fn ui(f: &mut Frame, app: &mut App) {
    ...
    let header = match &app.level {
        ViewLevel::SlotLevel => {
            let text = format!(
                "Solcap Scanner - Total Slots: {} | Press ↑/↓ to navigate, ←/→ for page jump, Enter to drill down, 'b' to go back, Ctrl+C to quit",
                app.slots.len()
            );
            Paragraph::new(text)
...
fn parse_path<P: AsRef<Path>>(path: P) -> Result<SolcapData, ScanError> {
    let path = path.as_ref();

    if !path.exists() {
        return Err(ScanError::PathNotFound(format!(
            "Path does not exist: {}",
            path.display()
        )));
    }
...
impl From<SolcapReaderError> for ScanError {
    fn from(err: SolcapReaderError) -> Self {
        ScanError::SolcapError(err)
    }
}

impl From<AgaveBhdReaderError> for ScanError {
    fn from(err: AgaveBhdReaderError) -> Self {
        ScanError::AgaveError(err)
    }
}
...
pub fn scan_solcap<P1: AsRef<Path>, P2: AsRef<Path>>(
    path1: P1,
    path2: P2,
) -> Result<(), ScanError> {
...
fn run_app<B: ratatui::backend::Backend>(
    ...
    loop {
        terminal.draw(|f| ui(f, app)).map_err(|e| {
            ScanError::IoError(io::Error::new(io::ErrorKind::Other, e.to_string()))
        })?;
